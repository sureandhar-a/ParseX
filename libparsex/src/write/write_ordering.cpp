...
void setSortedAttributes(
    ...
    std::ranges::stable_sort(sorted, {}, &std::pair<std::string, std::string>::first);
...
std::vector<FramePduMapping> sortedFramePduMappings(std::vector<FramePduMapping> mappings) {
    std::ranges::stable_sort(mappings, {}, &FramePduMapping::pduShortNameRef);
    return mappings;
}

std::vector<PduSignalMapping> sortedPduSignalMappings(
    std::vector<PduSignalMapping> mappings) {
    std::ranges::stable_sort(mappings, {}, &PduSignalMapping::signalShortNameRef);
    return mappings;
}

std::vector<ValueTableEntry> sortedValueTableEntries(std::vector<ValueTableEntry> entries) {
    std::ranges::stable_sort(entries, {}, &ValueTableEntry::value);
    return entries;
}
